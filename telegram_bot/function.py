...
async def get_health_recommendations(lat, lon, max_toke=512):
...
async def send_health_recommendations(lat, lon, number):
    message = await get_health_recommendations(lat, lon, max_tokens=250)
    await send_sms(number, message[:250])
